...
public class CashbookController {
	...
	public String removeCashbook(int cashbookId) {
	...
	public String modifyCashbook(Cashbook cashbook) { // 커맨드객체
		// System.out.println(cashbook);
		cashbookService.modifyCashbook(cashbook);
		int year = Integer.parseInt(cashbook.getCashbookDate().split("-")[0]);
		int month = Integer.parseInt(cashbook.getCashbookDate().split("-")[1]);
		int day = Integer.parseInt(cashbook.getCashbookDate().split("-")[2]);
		return "redirect:/admin/cashbookByDay?currentYear="+year+"&currentMonth="+month+"&currentDay="+day; 
	}
	...
	public String cashbookByDay(Model model,
		...
		currentYear = targetDay.get(Calendar.YEAR);
		currentMonth = targetDay.get(Calendar.MONTH)+1;
		currentDay = targetDay.get(Calendar.DATE);
